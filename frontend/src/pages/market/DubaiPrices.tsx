...
export default function DubaiPrices() {
  ...
  useEffect(() => {
    const fetchDubaiPrices = async () => {
      try {
        const res = await api.get('/market/dubai-prices').catch(() => ({ data: [] }));
        setData(Array.isArray(res.data) ? res.data : []);
      } catch {
        toast.error('Could not fetch Dubai live pricing index');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const columns = [
    ...
    { header: 'Local Price (AED)', accessor: (r: any) => `${r.local_market_price} AED`, className: 'font-mono font-black text-emerald-700' },
  ];

  if (isLoading) {
    return <PageLoader message="Querying Dubai Local Valuation Register..." />;
  }
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-on-surface">Dubai Local Prices Log</h1>
          <p className="text-sm text-on-surface-variant mt-0.5">Live Dubai Local (AED) historical rate audit feed</p>
        </div>
      ...
      <div className="bg-surface-container-lowest p-6 rounded-3xl border border-outline-variant shadow-sm space-y-4">
        {data.length === 0 ? (
          <div className="py-12 text-center text-slate-400 font-bold text-sm bg-slate-50/50 rounded-2xl border border-dashed border-outline-variant">
            No Dubai local rate logs recorded yet. Submit rates in the Daily Capture Sheet to build price intelligence.
          </div>
        ) : (
